...
def test_move_block():
    ...
    assert (blocks := block_move(blocks)) == expected
...
def test_compact_block() -> None:
    ...
    assert (compact_block(blocks)) == expected
...
def test_move_file() -> None:
    ...
    assert (blocks := file_move(disk_map, blocks, 9)) == expected

    expected = "0099.1117772...333.44.5555.6666.....8888.."

    assert (blocks := file_move(disk_map, blocks, 7)) == expected

    expected = "0099.111777244.333....5555.6666.....8888.."

    assert (blocks := file_move(disk_map, blocks, 4)) == expected

    expected = "00992111777.44.333....5555.6666.....8888.."

    assert (blocks := file_move(disk_map, blocks, 2)) == expected
