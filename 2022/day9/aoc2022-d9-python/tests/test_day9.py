...
def test_check_rope_is_touching() -> None:
    assert True == check_rope_is_touching(Rope((1, 2), (1, 1)))
    assert True == check_rope_is_touching(Rope((2, 1), (1, 2)))
    assert True == check_rope_is_touching(Rope((1, 1)))


def test_rope_move_tail() -> None:
    moved_right = rope_move_tail(Rope((1, 3), (1, 1)), DIRECTION_RIGHT)

    assert ((1, 2), True) == moved_right.tail
    assert ((1, 3), False) == moved_right.head
    assert True == moved_right[(1, 1)]

    moved_down = rope_move_tail(Rope((1, 1), (3, 1)), DIRECTION_DOWN)

    assert ((2, 1), True) == moved_down.tail
    assert ((1, 1), False) == moved_down.head
    assert True == moved_down[(3, 1)]

    moved_ur = rope_move_tail(Rope((3, 2), (1, 1)), DIRECTION_UP)
    ...
    moved_ur2 = rope_move_tail(Rope((2, 3), (1, 1)), DIRECTION_RIGHT)
...
def test_rope_move_head() -> None:
    moved_right = rope_move_head(Rope((1, 2), (1, 1)), DIRECTION_RIGHT)
...
def test_rope_move_motion() -> None:
    moved = rope_move_motion(Rope((0, 0)), "R", 4)
...
R 2""".strip()
    ...
    assert 13 == rope_calculate_visited(Rope((0, 0)), test_input)
